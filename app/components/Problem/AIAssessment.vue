      ...
      <v-card-item>
        <v-row align="center" no-gutters>
          <v-col cols="auto" class="d-flex align-center mr-4">
            <div>
              <div  :class="mobile ? 'text-body-2' : 'text-overline'">AI Assessment</div>
              <div v-if="!mobile" class="text-caption text-grey">Fact checks powered by Google Gemini</div>
            </div>
          </v-col>
          ...
          <v-col cols="auto" class="d-flex align-center">
            <div class="text-caption mr-2">
              <div v-if="problem.ai_assessment_status === 'pending'" class="d-flex align-center">
                <v-progress-circular indeterminate size="16" width="2" class="mr-2"></v-progress-circular>
                Submitting...
              </div>
              <div v-else-if="problem.ai_assessment_status === 'processing'" class="d-flex align-center">
                <v-progress-circular indeterminate size="16" width="2" class="mr-2"></v-progress-circular>
                Analyzing...
              </div>
              <div v-else-if="problem.ai_assessment_status === 'completed' && !isAssessmentExpanded" class="text-green-darken-1 font-weight-medium">
                Fact-check available
              </div>
              <div v-else-if="!isAssessmentExpanded" class="text-red">
                Analysis failed
              </div>
            </div>
